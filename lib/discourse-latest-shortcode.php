...
class DiscourseLatestShortcode {
	...
	public function discourse_latest( $atts ) {
		...
		$force = ! empty( $this->options['dclt_clear_topics_cache'] ) ? $this->options['dcclt_clear_topics_cache'] : null;

		$latest_topics     = $this->latest_topics( $discourse_url, $attributes['cache_duration'], $force );

		return $latest_topics ? $this->format_topics( $attributes, $latest_topics ) : '';
	}

	protected function latest_topics( $url, $cache_duration, $force ) {
		$latest_url = esc_url_raw( $url . '/latest.json' );

		$latest_topics = get_transient( 'dclt_latest_topics' );
		if ( empty( $latest_topics || $force ) ) {
			...
			set_transient( 'dclt_latest_topics', $latest_topics, $cache_duration * MINUTE_IN_SECONDS );
		}
